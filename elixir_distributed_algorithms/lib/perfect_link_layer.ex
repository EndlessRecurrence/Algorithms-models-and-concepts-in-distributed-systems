defmodule DistributedAlgorithmsApp.PerfectLinkLayer do
  ...
  def accept(port, process_index, nickname) do
    ...
    loop_acceptor(socket, process_index, nickname)
  end

  defp loop_acceptor(socket, process_index, nickname) do
    ...
    {:ok, pid} =
      DynamicSupervisor.start_child(PerfectLinkHandler.DynamicSupervisor, %{
        ...
        start: {PerfectLinkHandler, :start_link, [%{socket: client, process_index: process_index, owner: nickname}]},
        type: :worker,
        restart: :transient
      })
